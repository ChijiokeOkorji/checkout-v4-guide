...
This allows you to fetch the public key which would be used to encrypt your card encryption key. The keypair is autogenerated on startup, so even I don't know what it would be after every deployment. You would have to encrypt your developer encryption key on your device using Elliptic Curve Cryptography (ECC) before proceeding.
```
