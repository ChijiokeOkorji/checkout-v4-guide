...
This allows you to fetch the public key which would be used to encrypt your card encryption key. The keypair is autogenerated on startup, so even I don't know what it would be after every deployment.
```
...
This returns an encryption key that has already been encrypted with the server's static public key (normally you would manually encrypt your own encryption key with the public server's public key). This is just to allow quick flow demonstration, and in production environments, this endpoint would not exist, and you would have to encrypt your developer encryption key on your device using Elliptic Curve Cryptography (ECC) before proceeding.
```
